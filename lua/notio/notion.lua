...
local function canon_lhs(s)
  ...
  s = s:gsub("^<Space>", "<leader>")
       :gsub("<[mM]%-", "<M-")
       :gsub("<[aA]%-", "<M-")   -- Alt → Meta; swap to <A-> if you prefer
       :gsub("<[cC]%-", "<C-")
       :gsub("<[sS]%-", "<S-")
...
function Notion.new(opts)
  ...
  self.retries      = opts.retries or 2
  return self
end
...
-- Unified HTTP caller with timeout + small retry/backoff
  ...
  for _, page in ipairs(pages) do
    ...
    if name ~= "" then
      ...
      if lhs_text == "" then
        ...
        lhs_text = canon_lhs(lhs_text) -- canonicalize Name fallback too
      end
      ...
      if binding_fp ~= "" then
        local prev = by_binding[binding_fp]
        if not prev or prev.last_edited_time < info.last_edited_time then by_binding[binding_fp] = info end
      end
...
function Notion:query_by_name(database_id, name)
  ...
  if type(cmd) == "table" and cmd.type == "rich_text" then
